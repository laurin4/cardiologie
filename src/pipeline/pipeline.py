...
class ClinicalExtractionPipeline:
    ...
    def fieldnames(self) -> List[str]:
        return BASE_FIELDS + [f.name for f in self.task.fields]
    ...
    def run_report(self, report: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if not evidence["has_positive_evidence"]:
            ...
            status = STATUS_FAILED if any(
                str(e).startswith("llm_extraction_failed") for e in schema_errors
        ...
        row: Dict[str, Any] = {
            ...
            "llm_report_text_length": evidence["llm_report_text_length"],
...
def main() -> None:
    ...
    parser.add_argument(
        "--reports",
        default=None,
        help="Path to a reports CSV or a directory of .txt files.",
    )
    parser.add_argument("--output-dir", default=None, help="Output directory for results.")
    parser.add_argument(
        "--max-reports",
        default=None,
        help="Cap number of reports (int or 'all'). Overrides MAX_REPORTS env.",
    )
