...
parsed and validated against the schema before being returned. This single stage
replaces any per-task extraction/interpretation logic with a schema-driven call.
"""
...
def extract_entities(
    ...
    *,
    ...
    """
    llm_text = str(evidence_bundle.get("llm_report_text", "") or "")
    system_prompt = build_system_prompt(task)
    user_prompt = (
        "Rule-based evidence extracted from the clinical report "
        "(section-labelled snippets):\n\n"
        f"{llm_text}\n"
    )
    messages = [
